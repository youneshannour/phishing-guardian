...
class PlaybookEngine:
    ...
    def _build_synthesis(
        ...
        for step in steps:
            ...
            if step.plugin_id == "email_auth":
                findings = step.data.get("findings") or []
                if step.data.get("risk_level") in ("high", "medium"):
                    key_findings.append(
                        "Auth email: " + "; ".join(findings[:3])
                    )
                elif findings:
                    key_findings.append(f"Auth email OK (score {step.data.get('auth_score', 0)})")
            if step.plugin_id == "dns_lookup":
                recs = step.data.get("records") or {}
                a_count = len(recs.get("A") or [])
                mx_count = len(recs.get("MX") or [])
                key_findings.append(f"DNS: {a_count} A, {mx_count} MX")
            if step.plugin_id == "crtsh":
                key_findings.append(
                    f"crt.sh: {step.data.get('certificate_count', 0)} cert(s), "
                    f"{step.data.get('name_count', 0)} nom(s)"
                )
            if step.plugin_id == "phishing_analyze":
                label = step.data.get("label") or step.data.get("risk_level")
                score = step.data.get("score")
                if label or score is not None:
                    key_findings.append(
                        f"Analyse phishing: {label or 'n/a'}"
                        + (f" (score {score})" if score is not None else "")
                    )
            if step.plugin_id == "blocklist_check" and step.data.get("listed"):
                sources = ", ".join(step.data.get("sources_hit") or [])
                key_findings.append(f"Blocklist: listé ({sources or 'feed'})")
            if step.plugin_id == "hibp_osint" and (
                step.data.get("found") or step.data.get("data", {}).get("password_pwned")
            ):
                key_findings.append("OSINT léger: indicateur de fuite détecté")

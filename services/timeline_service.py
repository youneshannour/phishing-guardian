...
SOURCE_LABELS = {
    ...
    "crtsh": "crt.sh",
    "phishing_analyze": "Phishing",
    "blocklist_check": "Blocklists",
    "hibp_osint": "OSINT léger",
...
def _events_from_blocklist(step: Dict[str, Any]) -> List[Dict[str, Any]]:
    data = step.get("data") or {}
    if not data.get("listed"):
        return []
    now = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    sources = ", ".join(data.get("sources_hit") or []) or "feed"
    return [
        _make_event(
            occurred_at=now,
            date_precision="day",
            source="blocklist_check",
            event_type="reputation",
            title=f"Cible listée ({sources})",
            description="; ".join((data.get("findings") or [])[:2]),
            severity="critical",
        )
    ]


def _events_from_email_auth(step: Dict[str, Any]) -> List[Dict[str, Any]]:
    data = step.get("data") or {}
    ...
    return [
        _make_event(
            ...
            source="email_auth",
            event_type="reputation",
            title=f"Auth email faible (score {data.get('auth_score', '?')})",
            description="; ".join((data.get("findings") or [])[:3]),
            severity="high" if risk == "high" else "medium",
        )
    ]


def _events_from_phishing_analyze(step: Dict[str, Any]) -> List[Dict[str, Any]]:
    data = step.get("data") or {}
    label = (data.get("label") or "").lower()
    if label not in ("phishing", "suspect"):
        return []
    now = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    return [
        _make_event(
            occurred_at=now,
            date_precision="day",
            source="phishing_analyze",
            event_type="reputation",
            title=f"Analyse phishing : {label}",
            description=f"Score {data.get('score', '—')}",
            severity="critical" if label == "phishing" else "medium",
        )
    ]
...
def _events_from_step(step: Dict[str, Any], inv_completed: Optional[str]) -> List[Dict[str, Any]]:
    ...
    extractors = {
        ...
        "blocklist_check": _events_from_blocklist,
        "email_auth": _events_from_email_auth,
        "phishing_analyze": _events_from_phishing_analyze,
    }

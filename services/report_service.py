...
def _step_summary(step: Dict[str, Any]) -> str:
    ...
    if pid == "dns_lookup":
        ...
        return f"A {len(recs.get('A') or [])}, MX {len(recs.get('MX') or [])}"
    if pid == "email_auth":
        return f"Auth score {d.get('auth_score', '?')} — {'; '.join((d.get('findings') or [])[:2])}"
    if pid == "crtsh":
        return f"{d.get('certificate_count', 0)} cert(s), {d.get('name_count', 0)} nom(s)"
    if pid == "phishing_analyze":
        return f"{d.get('label') or d.get('risk_level') or 'n/a'} score={d.get('score', '—')}"
    if pid == "blocklist_check":
        if d.get("listed"):
            return f"Listé ({', '.join(d.get('sources_hit') or [])})"
        return "Non listé"
    if pid == "hibp_osint":
        return "Fuite détectée" if (d.get("found") or (d.get("data") or {}).get("password_pwned")) else "Pas de fuite"

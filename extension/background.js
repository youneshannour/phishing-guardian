...
async function getApiKey() {
  ...
  if (!apiKey) {
    try {
      const boot = await fetch(`${base}/api/auth/bootstrap`).then((r) => r.json());
      apiKey = boot.api_key || "";
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  if (msg.type === "PG_SELECTION") {
    const text = (msg.text || "").trim();
    if (text) {
      chrome.storage.local.set({ pendingTarget: text, pendingAction: "playbook" });
    }
  ...
  if (msg.type === "PG_ANALYZE_URL") {
    apiFetch("/api/analyze", {
      ...
      .then((data) => {
        ...
        const color =
          label === "phishing" ? "#ef4444" : label === "suspect" ? "#f59e0b" : "#22c55e";

...
class TestEntityResolver(unittest.TestCase):
    ...
    def test_suggest_playbook_for_email(self):
        self.assertEqual(suggest_playbook_id(EntityType.EMAIL), "phishing_triage")
...
class TestPlaybookEngine(unittest.TestCase):
    ...
    def test_suggest_returns_playbook(self):
        ...
        self.assertEqual(result["suggested_playbook_id"], "phishing_triage")
    ...
    def test_run_executes_plugins_in_order(self, mock_get_plugin):
        ...
        self.assertEqual(result.synthesis["tools_success"], 2)
        self.assertEqual(mock_plugin.run.call_count, 2)

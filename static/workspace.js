...
const WorkspaceUI = (() => {
 ...
 function getUser() {
 const sessionUser = (document.getElementById("pgSessionUser")?.textContent || "").trim();
 const stored = (localStorage.getItem(LS_USER) || "").trim();
 const user = (sessionUser || stored).toLowerCase();
 if (user && user !== "—" && !localStorage.getItem(LS_USER)) {

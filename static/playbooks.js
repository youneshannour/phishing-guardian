...
const PlaybooksUI = (() => {
 ...
 break;
 case "pbAddWatch":
 Promise.resolve(window.WatchUI?.addFromInvestigation(lastResult)).catch(() => {});
 break;
 case "pbAddWorkspace":
 Promise.resolve(window.WorkspaceUI?.addInvestigation(lastResult)).catch(() => {});
 break;
 case "pbExportJson":
 exportJson();
 break;
 ...
 data-id="${pb.id}">
 <div class="pb-playbook-icon">${esc(pb.icon || "PB")}</div>
 ...
 <div class="pb-empty-icon" aria-hidden="true"></div>

...
const WatchUI = (() => {
 ...
 function notifyNewAlerts(unreadCount, latestAlerts) {
 ...
 const newest = (latestAlerts || []).find((a) => !a.read) || latestAlerts?.[0];
 const title = newest?.title || "Nouvelle alerte Watch";
 const body = newest?.message || `${unreadCount - lastUnread} nouvelle(s) alerte(s)`;
 try {
 new Notification("Phishing Guardian — Watch", {
 body: `${title}\n${body}`.slice(0, 180),
 tag: "pg-watch-alert",
 });
 } catch (_) { /* ignore */ }
 ...
 <button type="button" class="pb-export-btn watch-delete-btn" data-id="${esc(w.id)}">Suppr.</button>
 </div>
 </div>`).join("");
